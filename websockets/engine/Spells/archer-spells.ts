...
export class PiercingShot extends Spell {
	...
	applyEffect(idUser: CharacterInstance, idTargets: CharacterInstance[], bus: EventBus): void {
		...
		const damage                  = resolvePhyDamage(raw, idUser, idTargets[0], bus, reducedRes);
...
export class RainOfArrows extends Spell {
	...
	applyEffect(idUser: CharacterInstance, idTargets: CharacterInstance[], bus: EventBus): void {
		...
		idTargets.forEach(target => {
			const damage = resolvePhyDamage(raw, idUser, target, bus, undefined, true);
...
export class PrecisionFocus extends Spell {
	...
	applyEffect(idUser: CharacterInstance, _idTargets: CharacterInstance[], bus: EventBus): void {
		...
		bus.pushSpell({ type: "buff_crit", targetUid: idUser.uid });
	}
}
